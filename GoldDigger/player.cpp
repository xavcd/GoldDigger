...
Vector3f Player::SimulateMove(bool front, bool back, bool left, bool right, bool sprint, float elapsedTime)
{
	...
	if (front)
	{
		...
		{
			delta.x += float(sin(yrotrad)) * elapsedTime * 15;
			delta.z -= float(cos(yrotrad)) * elapsedTime * 15;
		}
	}
